        ...
        for r in rooms:
            ...
            if type(room_index)==int:
                ...
                if r.dimension:
                    ...
                    # if not dim_index:
                    #     block_df = tess_dict.loc[
                    #         (tess_dict.loc[:, 'block_num']==room_block) & (tess_dict.loc[:, 'line_num']!=room_line), 'text'
                    #     ]
                    #     dim_index = block_df.index[block_df.apply(lambda x: re.sub(r'\D','',x)==re.sub(r'\D','',d))]
                    #     dim_index = dim_index[0] if dim_index.any() else None
                    if not dim_index:
